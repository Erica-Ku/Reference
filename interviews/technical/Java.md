# Java
  ...
  - Optional API
    - 가장 많이 발생하는 예외 중 하나가 NullPointerException이며 이를 피하기 위해서는 null 여부 검사가 필연적임
    - Java8부터 Optional<T>을 제공하여 null로 인한 예외가 발생하지 않도록 도와주고, Optional 클래스의 메서드를 통해 null을 컨트롤할 수 있음

  - 컬렉션 프레임워크
    - 다수의 데이터를 쉽고 효과적으로 관리할 수 있는 표준화된 방법을 제공하는 클래스의 집합을 의미함
    - 자바 컬렉션에는 List, Set, Map 인터페이스를 기준으로 여러 구현체가 존재하고 Stack, Queue 인터페이스도 존재함
    - List
      - 순서가 있는 데이터의 집합이며 중복을 허용함
      - 대표적인 구현체로는 ArrayList가 있고 이는 Vector를 개선한 것
      - 이외에도 LinkedList 등의 구현체가 있음
    - Set
      - 순서가 없는 데이터의 집합이며 중복을 허용하지 않음
      - 대표적인 구현체로는 HashSet이 있고 순서를 보장하기 위해 LinkedHashSet을 사용함
    - Map
      - 키와 값이 한 쌍으로 이루어져 있으며 키를 기준으로 중복을 허용하지 않고, 순서가 없음
      - 키의 순서를 보장하기 위해서는 LinkedHashMap을 사용함
    - Stack, Queue
